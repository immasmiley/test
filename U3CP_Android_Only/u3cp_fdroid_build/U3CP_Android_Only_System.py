...
class AndroidCommunicationSystem:
    ...
    def _handle_chat_message(self, message: AndroidMessage):
        """Handle chat messages and log them to a file."""
        try:
            chat_data = json.loads(message.payload.decode('utf-8'))
            log_message = f"Chat from {message.source_device}: {chat_data.get('message', '')}"
            print(f"💬 {log_message}")

            # Append the received message to a log file for confirmation
